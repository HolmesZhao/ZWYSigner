...
class MainView: NSView, URLSessionDataDelegate, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    func fileDropped(_ filename: String){
        ...
        case "ipa", "deb", "app", "xcarchive":
            let stringValue = InputFileText.stringValue
            ...
            if !stringValue.contains(filename) {
                InputFileText.stringValue = stringValue + "__" + filename
                inputFiles.append(filename)
            }
            break
    ...
    override func performDragOperation(_ sender: NSDraggingInfo) -> Bool {
        ...
        if !hasContent {
            ...
            alert.informativeText = "The file could not be found"
            alert.runModal()
            InputFileText.stringValue = ""
            inputFiles = []
        }
    ...
    func startSigning() {
        ...
        if saveDialog.runModal().rawValue == NSFileHandlingPanelOKButton {
            ...
            guard inputFiles.isEmpty == false else {
                let alert = NSAlert()
                alert.messageText = "Sign Error"
                alert.addButton(withTitle: "OK")
                alert.informativeText = "The file does not exist. Please drag again !"
                alert.runModal()
                outputFile = nil
                controlsEnabled(true)
                return
...
extension MainView {
    ...
    private func prepareForSignWithUDID() -> Promise<Bool> {
        ...
        let p = Promise<Bool> { resolver in
            ...
            //MARK: Create working temp folder
            let tempTask = Process().execute(mktempPath, workingDirectory: nil, arguments: ["-d","-t","zwysigner_files"])
            if tempTask.status != 0 {
            ...
            self.listAllCertificatesWithDisplayName(cer_display_name).then({ (certificates: [Certificate]) -> Promise<Certificate> in
                ...
                return self.saveFileWithContent(cerContent, filePath: tempPath.stringByAppendingPathComponent("zwysigner.cer"))
                ...
                return self.saveFileWithContent(cerContent, filePath: tempPath.stringByAppendingPathComponent("zwysigner.mobileprovision"))
        ...
        let p = Promise<[Device]> { resolver in
            ...
            provider!.request(endpoint) {
                ...
                case .failure(let error):
                    resolver.reject(error)
                    if let error1 = error as? APIProvider.Error,
                       case let .decodingError(data) = error1,
    ...
    private func registerdNewDevices(udids: [String], platform: Platform) -> Promise<[Device]> {
        ...
        let p = Promise<[Device]> { resolver in
            ...
            workingGroup.notify(queue: workingQueue) {
                if failedDevices.isEmpty == false {
                    DispatchQueue.main.async {
                        let alert = NSAlert()
                        alert.messageText = "Regist Devices Fail"
                        alert.addButton(withTitle: "OK")
                        alert.informativeText = failedDevices.description
                        alert.runModal()
                    }
                }
